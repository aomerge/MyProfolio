        ...
        <>
            {
                articles.map((item, index) => (
                    <article key={index} className="min-w-[600px] gap-10  px-4 sha z-20">
                        <a className=" border border-blue-800 h-[250px] flex bg-[#103d6a] hover:bg-[#f1f7fe] hover:text-blue-800 relative text-blue-100 p-6 rounded-md overflow-hidden">
                            <section className="flex flex-col  justify-between  z-20">
                                <h2 className="text-xl font-semibold">{item.title}</h2>
                                <p className="mt-2 font-light w-3/4 text-sm text-justify">{item.description}</p>
                            </section>
                            <section  className="absolute border-4 z-20 left-[300px] bottom-0 top-[-50px] right-0 border-blue-300 h-[400px] w-[400px] rounded-full "></section>
                            <div className=" absolu w-full top-0 left-0 h-full opacity-80 z-10">
                                <img
                                    className="absolute rotate-90 top-[-50px] left-[50px] h-[400px]"
                            ...
                            <div className="w-[260px] flex justify-center z-20 ">
