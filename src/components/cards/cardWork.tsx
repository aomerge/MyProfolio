...
const CardWork: React.FC<CardWorkProps> = ({ tags, id, title, body, image, href }) => {
  const orangeTwo = 'text-orange-500';


  return (
    <button key={id} className=" text-left rounded-lg px-5 py-2 border border-gray-500 border-spacing-5 max-sm:pb-20 max-sm:gap-8 gap-28">
      <div className="max-sm:w-full">
        <section className="flex justify-between w-full">
          <h2
            className={' max-sm:text-center text-2xl  font-semibold ' + orangeTwo}
          >
        ...
        <div className="mb-5">
          {tags &&
            tags.map((tag, index )=> {
              return <span key={index} className="text-gray-400 text-md mb-5"> {tag} /</span>;
            })}
        </div>
